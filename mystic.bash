...
EOA
	...
	# =============================================================================
	function mystic_pyenv_middle_check() {
		[[ "${PYENV_ACTIVATE_SHELL}" -eq 1 ]]
	}


	# =============================================================================
	# Renders and prints a widget displaying the current pyenv virtualenv
	#
	# Globals:
	#		gets:
	#			PYENV_VERSION
	...
	#
	# =============================================================================
	function mystic_pyenv_middle() {

		local output virtualenv

		virtualenv="$(mystic_ellipsify ${MYSTIC_MAXLENGTH_PWD} ${MYSTIC_GLYPH_ELLIPSIS} ${PYENV_VERSION})"
	...
	function mystic_evoke_light() {
		...
		function mkmid() {
			local active_git active_pyenv

			active_pyenv=$(mystic_check mystic_pyenv_middle_check)
			active_git=$(mystic_check mystic_git_middle_check)

			if [ ${active_git} ] || [ ${active_pyenv} ]; then
				mystic_middle_edge
			fi

			if [ "${active_pyenv}" = true ]; then
				mystic_pyenv_middle
			fi
